...
- 210 课程表2：拓扑排序，保存序列。   
- 621 任务调度器
- 269 外星文字典：map(set)(graph) + map(inDegree)，边加入graph要去重，防止inDegree多加了
- 444 重建序列：map(set)(graph) + map(inDegree)，BFS 遍历size保证层序遍历性(最短路模型)，加入graph要去重
